...
export function GeoField({ value, onChange }: Props) {
  ...
  return (
    <div>
      <input className={styles.input} type="number" placeholder="lon"
             value={value.lon ?? ''} onChange={(e) => onChange({ ...value, lon: num(e.target.value) })} />
      <input className={styles.input} type="number" placeholder="lat"
             value={value.lat ?? ''} onChange={(e) => onChange({ ...value, lat: num(e.target.value) })} />
      <input className={styles.input} type="number" placeholder="radius"
